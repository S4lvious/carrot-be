...
public class FatturaService {
    ...
    public void generaFattura(Ordine incarico, boolean inserisciMovimento ,boolean applicareRitenuta, BigDecimal ritenutaAcconto, LocalDate scadenza, String stato) {
        ...
        // Controlla l'esistenza di una fattura per l'ordine filtrando anche per utente
        ...
        // Recupero dell'utente emittente: in questo caso, usiamo l'utente corrente
        User emittente = currentUser;

        // Controllo dati obbligatori per l'emittente
        List<String> erroriEmittente = new ArrayList<>();
        if (emittente.getPartitaIva() == null) erroriEmittente.add("Partita IVA");
        if (emittente.getIndirizzo() == null) erroriEmittente.add("Indirizzo");
        if (emittente.getCap() == null) erroriEmittente.add("CAP");
        if (emittente.getCitta() == null) erroriEmittente.add("Città");
        if (emittente.getProvincia() == null) erroriEmittente.add("Provincia");
        if (!erroriEmittente.isEmpty()) {
            throw new IllegalStateException("Dati mancanti per l'emittente: " + String.join(", ", erroriEmittente));
        }
        ...
        if (!erroriCliente.isEmpty()) {
            throw new IllegalStateException("Dati mancanti per il cliente: " + String.join(", ", erroriCliente));
        }
        ...
        BigDecimal totaleIVA = incarico.getDettagliOrdine().stream()
            .map(d -> d.getPrezzoUnitario()
                    .multiply(BigDecimal.valueOf(d.getQuantita()))
                    .multiply(d.getProdotto().getAliquotaIVA().divide(BigDecimal.valueOf(100), MathContext.DECIMAL128))
            )
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        // Dati dell'emittente
        fattura.setNomeEmittente(emittente.getRagioneSociale() != null ? emittente.getRagioneSociale() : emittente.getNome() + " " + emittente.getCognome());
        fattura.setIndirizzoEmittente(emittente.getIndirizzo());
        fattura.setCapEmittente(emittente.getCap());
        fattura.setCittaEmittente(emittente.getCitta());
        fattura.setProvinciaEmittente(emittente.getProvincia());
        fattura.setPartitaIVAEmittente(emittente.getPartitaIva());
        fattura.setCodiceFiscaleEmittente(emittente.getCodiceFiscale());

        // Dati del cliente
        fattura.setNomeCliente(
            (cliente.getRagioneSociale() != null && !cliente.getRagioneSociale().trim().isEmpty())
        ...
        BigDecimal totaleDovuto = totaleLordo;
        if (applicareRitenuta) {
            BigDecimal importoRitenuta = totaleLordo.multiply(ritenutaAcconto.divide(BigDecimal.valueOf(100)));
            totaleDovuto = totaleLordo.subtract(importoRitenuta);
        }
